...
class ArViewState extends State<ArViewWidget> with WidgetsBindingObserver {
  ...
  Future<void> onArchitectWidgetCreated() async {
    ...
    if(sample.requiredExtensions.contains("application_model_pois")) {
      ...
      architectWidget.callJavascript("World.loadPoisFromJsonData(${jsonEncode(pois)});");
    }
    ...
    if(jsonObject["action"] != null){
      switch(jsonObject["action"]) {
        ...
        case "save_current_instant_target":
          ...
          architectWidget.callJavascript("World.saveCurrentInstantTargetToUrl(\"$filePath/SavedInstantTarget.wto\");");
          break;
        case "load_existing_instant_target":
          ...
          architectWidget.callJavascript("World.loadExistingInstantTargetFromUrl(\"$filePath/SavedInstantTarget.wto\",$augmentations);");
          break;  
      }
    }
  }
  ...
  Future<void> captureScreen() async {
    WikitudeResponse captureScreenResponse = await architectWidget.captureScreen(true, "");
    if(captureScreenResponse.success) {
      architectWidget.showAlert("Success", "Image saved in: ${captureScreenResponse.message}");
    } else {
